...
class CLVPMetric:
    def __init__(self, device='cpu', pretrained_path='/home/asif/tts_all/TTS_Evaluations/CVLP_Scores_TTS/ckpt/clvp.pth'):
        self.device = device
    ...
    def get_projection_for_files(self, files, verbose=True):
        with torch.no_grad():
            ...
            for file in tqdm(files, disable=not verbose):
                ...
                # print("projection append ok")
            return projections

    def compute_fd(self, gen_path, real_path, verbose=True):
        gen_files = list(pathlib.Path(gen_path).rglob('*.wav'))
        # print(self.get_projection_for_files(gen_files, verbose))
        gen_projections = torch.cat(self.get_projection_for_files(gen_files, verbose), dim=0)
        # print("1st file op doneeeeeeeeeeeeeeeeeeeeeeee")
        real_files = list(pathlib.Path(real_path).rglob('*.wav'))
        real_projections = torch.cat(self.get_projection_for_files(real_files, verbose), dim=0)

        # print(gen_projections.size())
        # print(real_projections.size())
        # assert False
